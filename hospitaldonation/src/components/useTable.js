...
const useStyles = makeStyles(theme => ({
    table: {
        ...
        '& thead th': {
            fontWeight: '200',
            color: theme.palette.primary.mainp,
...
export default function useTable( records, headCells,filterFn) {
    ...
    const TblContainer = props => (
        <Table className={classes.table} xs = {14} md={10}>
            {props.children}
        </Table>
    )
            ...
            <TableRow >
                {
                    headCells.map((headCell,index) => (
                        <TableCell key={index}
                            sortDirection={orderBy === headCell.id ? order : false} style={{ maxHeight:15, height:10  }}>
    ...
    const donorDataAfterPagingAndSorting = () => {
        return stableSort(filterFn.fn(records), getComparator(order, orderBy))
            .slice(page * rowsPerPage, (page + 1) * rowsPerPage)
    }
    return {
